...
class WP_Job_Manager_Job_Tags extends WPJM_Updater {
	...
	public function get_job_tag_list( $job_id ) {
		...
		$job_tags = wp_get_post_terms( $job_id, 'job_listing_tag' );		
		foreach($job_tags as $job_tag){
			//$tags_array[] = '<a rel="tag" href="'.get_term_link( $job_tag ).'/?custom_tag='.$job_tag->term_id.'">'.$job_tag->name.'</a>';
			$tags_array[] = '<a rel="tag" href="'.get_term_link( $job_tag ).'">'.$job_tag->name.'</a>';
			}
		$terms = implode(',',$tags_array);
