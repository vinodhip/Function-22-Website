...
class WP_Resume_Manager_Form_Submit_Resume extends WP_Job_Manager_Form {
	...
	public function submit_handler() {
		try {
			...
			// Update the job
			$this->save_resume( $values['resume_fields']['mc4wp-candidate_name'], $values['resume_fields']['resume_content'], $this->resume_id ? '' : 'preview', $values );
...
//			print_r($post);
//			echo '</pre>';
			if ( in_array( $post->post_status, array( 'preview', 'expired' ) ) ) {

				// Reset expirey
				delete_post_meta( $post->ID, '_resume_expires' );

				// Update listing
				$update_resume                = array();
				$update_resume['ID']          = $post->ID;
...
<?php /*?>			<form method="get" id="cutom_submit_data">
				...
				<input type="hidden" name="step" value="<?php echo esc_attr( 2 ); ?>" />
			</form>
<?php */?>		
			<?php //wp_safe_redirect( esc_url_raw( add_query_arg( array( 'step' => $this->step, 'job_id' => $this->job_id, 'resume_id' => $this->resume_id ) ) ) );	
...
<?php /*?>			<form method="post" id="resume_preview">
